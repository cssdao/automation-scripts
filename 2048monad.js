(function () {
    // ==================== 用户配置区 ====================
    var MAX_GRID = 2048;      // 目标块，达到后停止    可调整
    var SEARCH_TIME = 500;         // 每次移动间隔时间(ms)   可调整
    ...
    checkIntervalId = setInterval(function () {
        var score = getScore();
        const maxgrid = getMaxGrid();
        // 如果游戏结束或达到目标分数
        if (gameLost() || maxgrid >= MAX_GRID) {
            // 停止所有定时器
            ...
            // 判断终止原因
            var stopReason = gameLost() ? "游戏结束" : "最大块达到" + MAX_GRID;
    ...
    function nextMove() {
        // 检查游戏是否已结束或分数达到目标
        if (gameLost() || getMaxGrid() >= MAX_GRID) {
            return;
        }
    ...
    function getGrid() {
        ...
        const gridContainer = document.getElementsByClassName("aspect-square")[0]
        if (!gridContainer) return grid;
    ...
    function gameLost() {
        ...
        const gameOverElement = document.querySelector(".text-2xl.font-bold.mb-4");
    ...
    function getScore() {
        ...
        const scoreElement = document.getElementsByClassName("retro-number")[0]
